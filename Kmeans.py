...
'''
D = [float(0) for i in range(row_num)]


cumsum = [float(0) for i in range(row_num)]


i = mean_arr.pop()

for col in range(col_num):
    multi_list2[0][col] = multi_list1[i][col]
...
for j in range(1, cluster_num):
    ...
    for i in range(row_num):
        D[i] = math.pow(mindist(multi_list1[i], multi_list2,
                        cluster_num), 2)
        sum_ = sum_ + D[i]
        cumsum[i] = sum_
  
    for k in range(row_num):
    	cumsum[k] = cumsum[k] / sum_
    
    random_ = random.random()
    for i in range(row_num):
    	if cumsum[i] > random_:
           for col in range(col_num):
    	       multi_list2[j][col] = multi_list1[i][col]
           break
'''
